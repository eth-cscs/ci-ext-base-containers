...
for spackver in "v0.20.3" "v0.21.0" ; do
    for baseimg in docker.io/ubuntu:22.04 ; do
        ...
        DOCKER_TAG=${REMOTE}/spack:${SPACK_DOCKER_TAG}-${OS_DOCKER_TAG}-$(uname -m)
        BASE_TAG_NAME=${REMOTE}/spack:base-${OS_DOCKER_TAG}-$(uname -m)
        ...
        for cudaver in "11.7.1" ; do
            ...
            CUDA_BASE_TAG_NAME=${REMOTE}/spack:base-cuda${cudaver}-${OS_DOCKER_TAG}-$(uname -m)
            CUDA_DOCKER_TAG=${REMOTE}/spack:${SPACK_DOCKER_TAG}-cuda${cudaver}-${OS_DOCKER_TAG}-$(uname -m)
